...
(function (global) {
  ...
  templates.select_option = async function (deps, params) {
    ...
    // 层级选项可能需要父节点 hover 后才渲染子节点。
    // 黑盒模式仅依赖运行时浮层和 ARIA 语义，不绑定任何组件库实现。
    var findExpandableParents = null;
    ...
    for (var retry = 0; retry < 3 && !optionEl; retry++) {
      ...
      if (!optionEl && runtimeInteraction && runtimeInteraction.reveal === "hover" && vc.mouseHover) {
    ...
    if (optionEl.x && optionEl.y) {
      var optionClick = await vc.clickAtPoint(evalInPage, optionEl.x, optionEl.y, runtimeInteraction || undefined);
      if (!optionClick.ok) {
    ...
    if (verifyValue && verifyValue.value && verifyValue.value.indexOf(expectedOptionText) === -1) {
      if (optionEl.x && optionEl.y) {
        ...
        await sleep(STEP_DELAY + 200);
        // 用 CDP 真实鼠标点击选项坐标
        var retryOptionClick = await vc.clickAtPoint(evalInPage, optionEl.x, optionEl.y, runtimeInteraction || undefined);
  ...
  templates.select_multi = async function (deps, params) {
    ...
    var findExpandableParentsForMulti = null;
    for (var i = 0; i < params.options.length; i++) {
      ...
      if (!ok && runtimeInteraction && runtimeInteraction.reveal === "hover" && vc.mouseHover) {
      ...
      if (ok && foundOpt && typeof foundOpt.x === "number" && typeof foundOpt.y === "number") {
        var multiOptionClick = await vc.clickAtPoint(evalInPage, foundOpt.x, foundOpt.y, runtimeInteraction || undefined);
    ...
    if (params.applyAfterSelection) {
      if (!runtimeInteraction || !runtimeInteraction.applySelector) {
        return { ok: false, result: "未观察到可安全点击的提交控件；请先用 click_button 或 find_element 定位当前页面的搜索/提交按钮", pageState: await getPageState(evalInPage) };
      }
      var applyResult = await vc.clickBySelector(evalInPage, runtimeInteraction.applySelector);
      if (!applyResult.ok) {
        return { ok: false, result: "已选择标签，但提交筛选失败: " + (applyResult.error || runtimeInteraction.applySelector), pageState: await getPageState(evalInPage) };
      }
