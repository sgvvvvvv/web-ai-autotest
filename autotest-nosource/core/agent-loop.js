...
(function (global) {
  ...
  function createAgentLoop(deps) {
    ...
    function getVisualFocusOptions() {
      var current = null;
      for (var i = 0; i < state.testCases.length; i++) {
        if ((state.testCases[i].id || "") === state.currentTcId || state.testCases[i].status === "testing") {
          current = state.testCases[i];
          break;
        }
      }
      if (!current) {
        for (var j = 0; j < state.testCases.length; j++) {
          if (state.testCases[j].status === "pending") { current = state.testCases[j]; break; }
        }
      }
      if (!current) return { maxElements: 18, autoCrop: false };

      var text = [current.title, current.page, current.preconditions, current.steps, current.expected, current.text]
        .filter(Boolean).join(" ");
      var ignored = { "测试": true, "页面": true, "功能": true, "操作": true, "点击": true, "输入": true, "验证": true, "预期": true, "结果": true, "是否": true, "可以": true, "需要": true, "进行": true, "当前": true, "通过": true, "失败": true, "数据": true, "显示": true, "正常": true };
      var terms = {};
      var chunks = text.match(/[a-zA-Z0-9_-]{2,}|[\u4e00-\u9fff]{2,}/g) || [];
      for (var ci = 0; ci < chunks.length; ci++) {
        var chunk = chunks[ci].toLowerCase();
        if (!ignored[chunk]) terms[chunk] = true;
        if (/^[\u4e00-\u9fff]+$/.test(chunk)) {
          for (var size = 2; size <= Math.min(4, chunk.length); size++) {
            for (var start = 0; start <= chunk.length - size; start++) {
              var piece = chunk.slice(start, start + size);
              if (!ignored[piece]) terms[piece] = true;
            }
          }
        }
      }
      return { focusTerms: Object.keys(terms).slice(0, 32), maxElements: 14, autoCrop: true };
    }
    ...
    function buildRunArtifacts() {
      return {
        coverage: state.coverage ? state.coverage.summary(state.testCases) : null,
        recentActions: state.history.slice(-40),
    ...
    async function getSnapshot() {
      ...
      if (state.coverage) {
        state.coverage.observeSnapshot(state.snapshot);
        if (deps.onCoverage) deps.onCoverage(state.coverage.summary(state.testCases));
      }
    ...
    async function executeAction(toolCall) {
      ...
      switch (name) {
        case "click":
          ...
          if (annotationMatch) {
            ...
            result.result = annotationClick.ok
              ? "已将截图标注 #" + annotationLabel + " 映射为真实点击"
            ...
            try {
              ...
              if (foundEls.length === 0) {
                ...
                result.result = "未找到匹配元素: findBy=" + findBy + ", value=" + findValue +
                  "\n建议：1) 检查当前 DOM 快照中的文本、label、placeholder 和 elementRef 2) 改用 text_contains 或 label 查找 3) 使用 screenshot 或 verify_ui 观察真实页面。";
              } else {
        ...
        case "assert":
          ...
          if (assertionAccepted && state.coverage) state.coverage.recordAssertion(assertion, assertionCheck.testCaseId);
          if (assertionAccepted && deps.onAssertion) deps.onAssertion(assertion, state.assertions, state.testCases, state.coverage && state.coverage.summary(state.testCases));
        ...
        case "screenshot":
          // 截取与当前 TC 关联的标注截图；无关联命中时才回退受上限约束的全局候选。
          ...
          try {
            ...
            var screenshotData = await global.AIFT_VisualController.captureAnnotatedForAI(deps.evalInPage, {
              clip: args.clip || null,
              focusTerms: getVisualFocusOptions().focusTerms || [],
              maxElements: 14,
              autoCrop: !args.clip,
            });
            ...
            result.result = "标注截图成功（" + screenshotData.width + "x" + screenshotData.height + "），" +
              "共标注 " + state.annotatedElements.length + " 个" + (screenshotData.focused ? "当前用例相关" : "可交互") + "元素" + (screenshotData.cropped ? "，已裁剪到关联区域" : "") + "。" +
              "优先使用预设模板或 click(selector) 操作。如需验证页面展示效果请使用 verify_ui。";
            result.screenshot = screenshotData;
            log("📸 标注截图: " + screenshotData.width + "x" + screenshotData.height + ", " + state.annotatedElements.length + " 个元素" + (screenshotData.cropped ? "，局部裁剪" : ""));
            // 通知面板展示截图
        ...
        case "verify_ui":
          ...
          try {
            await global.AIFT_VisualController.ensureAttached(deps.tabId);
            // 截取当前 TC 相关区域的纯净截图；找不到相关控件时自动回退整个视口。
            var verifyFocus = getVisualFocusOptions();
            var verifyScreenshot = await global.AIFT_VisualController.captureAnnotatedForAI(deps.evalInPage, {
              clip: args.clip || null,
              focusTerms: verifyFocus.focusTerms || [],
              maxElements: verifyFocus.maxElements || 14,
              autoCrop: !args.clip,
              drawOverlay: false,
            });
            result.result = "UI 验证截图已捕获（" + verifyScreenshot.width + "x" + verifyScreenshot.height + "）" + (verifyScreenshot.cropped ? "，已聚焦当前用例区域" : "") + "。" +
              "请仔细检查截图中的页面展示：\n" +
              "1. 布局是否正常（无错位、重叠、溢出）\n" +
            ...
            log("🔍 UI 验证截图: " + verifyScreenshot.width + "x" + verifyScreenshot.height + (verifyScreenshot.cropped ? "，局部裁剪" : ""));
            // 通知面板展示截图
            if (deps.onStream) deps.onStream("screenshot", JSON.stringify({
        ...
        var planMessages = [
          {
            role: "system",
            content: [
              "你是一个前端测试策略规划专家。你的任务是分析当前页面的运行时状态，",
              "为后续的自动化测试制定最优执行策略。",
              "",
              "## 输出要求",
              "请输出结构化的测试策略，包括：",
    ...
    /**
     * 运行 agent loop
     * @param {Object} params - { requirement, testCases }
     */
    async function run(params) {
      ...
      state.coverage = global.AIFT_CoverageTracker && global.AIFT_CoverageTracker.create
        ? global.AIFT_CoverageTracker.create() : null;
      ...
      try {
        log("黑盒模式：仅使用运行时 DOM、截图、导航和网络响应。");
        ...
        if (deps.config.visionSupported && global.AIFT_VisualController) {
          try {
            state.screenshot = await global.AIFT_VisualController.captureAnnotatedForAI(deps.evalInPage, getVisualFocusOptions());
            state.annotatedElements = state.screenshot.elements || [];
            log("📸 初始标注截图: " + state.screenshot.width + "x" + state.screenshot.height + ", " + state.annotatedElements.length + " 个元素" + (state.screenshot.cropped ? "，局部裁剪" : ""));
            // 通知面板展示初始截图
            if (deps.onStream) deps.onStream("screenshot", JSON.stringify({
        ...
        while (!state.aborted && !state.finished) {
          ...
          var messages = global.AIFT_PromptBuilder.buildMessages({
            ...
            runtimeInventory: params.runtimeInventory || null,
         ...
         state.conversationHistory.push({
          ...
          for (var t = 0; t < toolCalls.length; t++) {
            ...
            if (state.coverage) {
              state.coverage.recordAction(execResult.action, execResult.args, state.snapshot, execResult.ok);
              if (deps.onCoverage) deps.onCoverage(state.coverage.summary(state.testCases));
            }
            recordTrace({
              type: "tool_result",
          ...
          if (stalledFailure && state.currentTcId) {
            ...
            if (stalledFailure.kind === "warning") {
              state.loopWarning = "⚠️ 当前用例的交互未产生页面变化：" + stalledReason + "（" + stalledFailure.target + "）。请重新观察 DOM、截图或定位真实状态控件，禁止继续重复点击。";
          ...
          if (!state.finished && !state.aborted) {
            ...
            if (needDiagnosticScreenshot && deps.config.visionSupported && global.AIFT_VisualController) {
              try {
                state.screenshot = await global.AIFT_VisualController.captureAnnotatedForAI(deps.evalInPage, getVisualFocusOptions());
                state.annotatedElements = state.screenshot.elements || [];
                log("📸 失败诊断标注截图: " + state.screenshot.width + "x" + state.screenshot.height + ", " + state.annotatedElements.length + " 个元素" + (state.screenshot.cropped ? "，局部裁剪" : ""));
                // 通知面板展示截图
                if (deps.onStream) deps.onStream("screenshot", JSON.stringify({
                  dataUrl: state.screenshot.dataUrl,
                  width: state.screenshot.width,
        ...
        if (state.aborted) {
          ...
          setStatus("已中止");
          if (deps.onFinish) deps.onFinish(abortResult, abortSummary, state.assertions, state.testCases, buildRunArtifacts());
        } else {
          ...
          if (deps.onFinish) deps.onFinish(finalResult, finalSummary, state.assertions, state.testCases, buildRunArtifacts());
        }

      } catch (e) {
        log("Agent loop 异常: " + (e.message || e));
        setStatus("异常终止");
        if (deps.onFinish) deps.onFinish("error", "异常: " + (e.message || e), state.assertions, state.testCases, buildRunArtifacts());
      } finally {
        state.running = false;
        // 停止网络录制
        if (global.AIFT_NetworkRecorder) {
