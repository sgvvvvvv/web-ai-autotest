...
- 每条用例的断言被接受后都会进入 Skill 反思队列；反思只分析交互操作，不把用例本身沉淀为 Skill。AI 判断存在困难操作时，按下拉选择、输入框输入、按钮点击等操作粒度生成 Skill，并保存到 `chrome.storage.local`；没有困难操作时返回空数组。反思任务串行执行，避免并发 AI 调用，也不因上一条反思未完成而丢弃后续用例。
