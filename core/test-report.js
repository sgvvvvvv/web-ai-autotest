...
(function (global) {
  ...
  function calculateStats(testCases) {
    var stats = { total: 0, passed: 0, failed: 0, skipped: 0, pending: 0, testing: 0 };
    ...
    stats.passRate = stats.tested > 0 ? Math.round(stats.passed / stats.tested * 100) : 0;
    return stats;
  }
  ...
  function buildReport(input) {
    ...
    return {
      ...
      assertions: (input.assertions || []).map(function(assertion) {
        return { description: assertion.description || "", passed: !!assertion.passed };
      }),
  ...
  function buildMarkdown(report) {
    ...
    lines.push("- 用例总数: " + stats.total + "；通过: " + stats.passed + "；失败: " + stats.failed + "；跳过: " + stats.skipped + "；未完成: " + (stats.pending + stats.testing));
    lines.push("- 已测通过率: " + stats.passRate + "%");
    lines.push("");
    if (report.summary) {
      lines.push("## AI 总结");
      lines.push("");
    ...
    if ((report.assertions || []).length > 0) {
      ...
      report.assertions.forEach(function(assertion) {
        lines.push("- " + (assertion.passed ? "✅" : "❌") + " " + assertion.description);
      });
    }
