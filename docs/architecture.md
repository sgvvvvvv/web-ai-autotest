...
│   ├── skill-manager.js         # 本地操作级 Skill 存储、匹配和使用统计
│   └── skill-learner.js         # 用例完成后的 AI 反思与 Skill 沉淀
└── devtools/
    ├── icon-16.png
    ├── icon-48.png
...
flowchart TB
    ...
    R --> F[用例完成后反思队列]
    F --> L[AI 判断困难操作]
    L -->|有可复用经验| K[保存操作级 Skill 到 chrome.storage.local]
    L -->|无困难操作| C
    K --> C
```
...
- 上下文截断策略：动作历史仅保留最近 20 条，更早的记录以摘要形式省略；无工具调用的纯文本回复截断至 1000 字符后存入对话历史；推理重复循环中断时仅保留 500 字符摘要，不完整回传被判定为重复的内容。避免旧上下文反复注入导致模型复述历史提示词。
...
- 每条用例的断言被接受后都会进入 Skill 反思队列；反思只分析交互操作，不把用例本身沉淀为 Skill。AI 判断存在困难操作时，按下拉选择、输入框输入、按钮点击等操作粒度生成 Skill，并保存到 `chrome.storage.local`；没有困难操作时返回空数组。反思任务串行执行，避免并发 AI 调用，也不因上一条反思未完成而丢弃后续用例。反思 AI 使用 `AIFT_AIClient.chat()` 返回的 `message.content` 解析结果，Agent 在生成最终报告前等待本轮已触发的反思任务完成。
